...
function renderFactureForm(body) {
  ...
  body.innerHTML = `
    ...
    ${_factureEdit !== null && (getAllFactures()[_factureEdit]?.statut === 'Facturé') ? '<button onclick="var f=getAllFactures()[_factureEdit];if(f)envoyerRelanceFacture(f)" style="width:100%;padding:12px;border-radius:10px;border:none;background:linear-gradient(135deg,#F59E0B,#D97706);color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;margin-bottom:10px">🔔 Relancer le client</button>' : ''}
