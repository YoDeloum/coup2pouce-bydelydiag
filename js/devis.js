...
var DEVIS_DIAGNOSTICS_LIST = ['DPE','DPE Projeté','DPE Immeuble','Amiante','Prélèvement Amiante','Plomb','Prélèvement Plomb','Électricité','Gaz','Termites','ERP','Carrez','Boutin','Avant travaux','Avant démolition','Audit énergétique','Frais déplacement'];
...
function renderDevisForm(body) {
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
      <button onclick="convertirEnFacture()" style="padding:12px;border-radius:10px;border:2px solid #5B21B6;background:#fff;color:#5B21B6;font-size:13px;font-weight:700;font-family:inherit;cursor:${_devisEdit===null?'not-allowed':'pointer'};opacity:${_devisEdit===null?.4:1}" ${_devisEdit===null?'disabled':''}>📋 Convertir facture</button>
      <button onclick="convertirEnMission()" style="padding:12px;border-radius:10px;border:2px solid #E8650A;background:#fff;color:#E8650A;font-size:13px;font-weight:700;font-family:inherit;cursor:${_devisEdit===null?'not-allowed':'pointer'};opacity:${_devisEdit===null?.4:1}" ${_devisEdit===null?'disabled':''}>🏠 Créer mission</button>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
      <button onclick="ouvrirRelance()" style="padding:12px;border-radius:10px;border:2px solid #6366F1;background:#fff;color:#6366F1;font-size:13px;font-weight:700;font-family:inherit;cursor:${_devisEdit===null?'not-allowed':'pointer'};opacity:${_devisEdit===null?.4:1}" ${_devisEdit===null?'disabled':''}>📨 Relancer client</button>
      <button onclick="ouvrirEnvoiDocuments()" style="padding:12px;border-radius:10px;border:2px solid #0891B2;background:#fff;color:#0891B2;font-size:13px;font-weight:700;font-family:inherit;cursor:${_devisEdit===null?'not-allowed':'pointer'};opacity:${_devisEdit===null?.4:1}" ${_devisEdit===null?'disabled':''}>📄 Docs réglementaires</button>
    </div>
...
function envoyerMailDevis(devis) {
  ...
  .then(function(res) {
    ...
    return res.text().then(function(text) {
      if (!text || !text.trim()) throw new Error('Réponse vide du serveur (HTTP ' + status + ') — consultez Netlify → Functions → Logs');
